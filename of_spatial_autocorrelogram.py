...
def calculate_autocorrelograms(cluster_spike_data, max_spikes):
    # computes the 1 dimensional and 2 dimensional spatial autocorrelogram for a single cell
    ...
    # only use the first n spikes as it takes up a lot of memory
    ...
    autocorrelogram_2d, xedges, yedges = np.histogram2d(rotated_pos_from_spikes_x, rotated_pos_from_spikes_y,
...
def plot_egocentric_spike_spatial_autocorrelogram(spike_data, position_data, output_path, return_spike_data=False):
    ...
    for cluster_index, cluster_id in enumerate(spike_data.cluster_id):
        ...
        if len(firing_times_cluster)>1:
            ...
            shuffled_spatial_firing = cluster_spike_data.copy()
            ...
            shuffled_spatial_firing["firing_times"].iloc[0] = np.sort(shuffled_firing_times)
            shuffled_spatial_firing = PostSorting.open_field_spatial_firing.find_firing_location_indices(shuffled_spatial_firing, position_data)

            autocorrelogram_2d, autocorrelogram_1d, bin_edges = calculate_autocorrelograms(cluster_spike_data, max_spikes=50000)
            shuffle_autocorrelogram_2d, shuffle_autocorrelogram_1d, bin_edges = calculate_autocorrelograms(shuffled_spatial_firing, max_spikes=50000)

            diff_autocorrelogram_2d = autocorrelogram_2d-shuffle_autocorrelogram_2d
            autocorrelogram_2d = min_max_normalize(autocorrelogram_2d)
            shuffle_autocorrelogram_2d = min_max_normalize(shuffle_autocorrelogram_2d)
            diff_autocorrelogram_2d = min_max_normalize(diff_autocorrelogram_2d)

            # make the figure
            fig = plt.figure(figsize=(4,4))
            ax = fig.add_subplot(1, 1, 1)  # specify (nrows, ncols, axnum)
            cmap = plt.cm.get_cmap("inferno")
            c = ax.imshow(autocorrelogram_2d, interpolation='none', cmap=cmap, vmin=0, vmax=1, origin='lower')
            ...
            ax = fig.add_subplot(1, 1, 1)  # specify (nrows, ncols, axnum)
            cmap = plt.cm.get_cmap("inferno")
            c = ax.imshow(autocorrelogram_2d, interpolation='none', cmap=cmap, vmin=0, vmax=0.1, origin='lower')
            clb = fig.colorbar(c, ax=ax, shrink=0.5)
            ...
            # make an enhanced contrast figure of the shuffle
            fig = plt.figure(figsize=(4,4))
            ax = fig.add_subplot(1, 1, 1)  # specify (nrows, ncols, axnum)
            cmap = plt.cm.get_cmap("inferno")
            c = ax.imshow(shuffle_autocorrelogram_2d, interpolation='none', cmap=cmap, vmin=0, vmax=0.1, origin='lower')
            clb = fig.colorbar(c, ax=ax, shrink=0.5)
            clb.mappable.set_clim(0, 0.1)
            plt.ylabel('Y lag (cm)', fontsize=20, labelpad = 10)
            plt.xlabel('X lag (cm)', fontsize=20, labelpad = 10)
            ...
            plt.subplots_adjust(hspace = .35, wspace = .35,  bottom = 0.2, left = 0.2, right = 0.87, top = 0.92)
            plt.tight_layout(pad=0.4, w_pad=0.5, h_pad=1.0)
            plt.savefig(save_path + '/' + spike_data.session_id.iloc[cluster_index] + '_egocentric_spatial_autocorrelogram_shuffle_Cluster_' + str(cluster_id) + '_enhanced_constrast.png', dpi=200)
            plt.close()

            # make an enhanced contrast figure of the shuffle minus the cell
            fig = plt.figure(figsize=(4,4))
            ax = fig.add_subplot(1, 1, 1)  # specify (nrows, ncols, axnum)
            cmap = plt.cm.get_cmap("inferno")
            ...
            plt.subplots_adjust(hspace = .35, wspace = .35,  bottom = 0.2, left = 0.2, right = 0.87, top = 0.92)
            plt.tight_layout(pad=0.4, w_pad=0.5, h_pad=1.0)
            plt.savefig(save_path + '/' + spike_data.session_id.iloc[cluster_index] + '_egocentric_spatial_autocorrelogram_diff_Cluster_' + str(cluster_id) + '_enhanced_constrast.png', dpi=200)
            plt.close()
...
def process_recordings(of_recording_path_list):
    ...
    for recording in of_recording_path_list:
        ...
        try:
            ...
            spike_data = plot_egocentric_spike_spatial_autocorrelogram(spike_data, position_data, output_path, return_spike_data=True)
...
def plot_peak_histogram(of_path_list, save_path):
    ...
    grid_cells_peaks = Edmond.plot_utility2.pandas_collumn_to_numpy_array(grid_cells["spatial_autocorr_peak_cm"])
    non_grid_cells_peaks = Edmond.plot_utility2.pandas_collumn_to_numpy_array(non_grid_cells["spatial_autocorr_peak_cm"])
