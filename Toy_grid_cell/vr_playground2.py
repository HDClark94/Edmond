...
def plot_linear_grid_cell_lomb_anchored(n_trials, save_path, bin_size_cm=1, sampling_rate=100, avg_speed_cmps=10, p_scalar=1):
    ...
    for n, track_length in enumerate(track_lengths):
        for m, offset in enumerate(offsets):
            ...
            gauss_kernel = Gaussian1DKernel(stddev=5) # sigma = 5ms
            spike_locations = locations[spikes_at_locations==1]
            instantaneous_firing_rate, location_edges = np.histogram(spike_locations, bins=locations)
            instantaneous_firing_rate = convolve(instantaneous_firing_rate, gauss_kernel)
            instantaneous_firing_rate = moving_sum(instantaneous_firing_rate, window=2)/2
            location_centres = 0.5*(location_edges[1:]+location_edges[:-1])
            instantaneous_firing_rate = np.append(instantaneous_firing_rate, np.zeros(len(location_centres)-len(instantaneous_firing_rate)))

            ds_factor = 2
            set_fr = downsample(instantaneous_firing_rate, int(len(instantaneous_firing_rate)/ds_factor))
            set_elapsed_distance = downsample(location_centres, int(len(location_centres)/ds_factor))/track_length
            ls = LombScargle(set_elapsed_distance, set_fr)
            step = 0.001
            frequency = np.arange(0.1, 10+step, step)
            power = ls.power(frequency)
            max_int_SNR = get_max_int_SNR(frequency, power)
            max_SNR, max_SNR_freq = get_max_SNR(frequency, power)
            max_int_SNR_text = "iSNR: " + reduce_digits(np.round(max_int_SNR, decimals=2), n_digits=6)
            ...
            #frequency, power = ls.autopower(minimum_frequency=0.1, maximum_frequency=10, samples_per_peak=5)
            ...
            sig_distances = get_sig_distances(power, distance, far)
            #for i in range(len(sig_distances)):
            #    axes[m,n].axvline(x=sig_distances[i], ymin=0.5, ymax=1, color="red")
...
def plot_linear_grid_cell_lomb_null(n_trials, save_path, bin_size_cm=1, sampling_rate=100, avg_speed_cmps=10, p_scalar=1):
    ...
    for n, track_length in enumerate(track_lengths):
        for m, offset in enumerate(offsets):
            ...
            location_centres = 0.5*(location_edges[1:]+location_edges[:-1])
            instantaneous_firing_rate = np.append(instantaneous_firing_rate, np.zeros(len(location_centres)-len(instantaneous_firing_rate)))
            set_fr = instantaneous_firing_rate
            set_elapsed_distance = location_centres
...
def plot_linear_grid_cell_lomb(n_trials, save_path, bin_size_cm=1, sampling_rate=100, avg_speed_cmps=10, p_scalar=1):
    ...
    for n, track_length in enumerate(track_lengths):
        for m, offset in enumerate(offsets):
            ...
            '''
            numerator, bin_edges = np.histogram(locations, bins=10*int(track_length/1)*n_trials, range=(0, track_length*n_trials), weights=spikes_at_locations)
            denominator, _ = np.histogram(locations, bins=10*int(track_length/1)*n_trials, range=(0, track_length*n_trials))
