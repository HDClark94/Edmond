...
import Edmond.Ramp_analysis.Integrated_ramp_analysis.Python_PostSorting.plot_utility
import Edmond.Ramp_analysis.Integrated_ramp_analysis.Python_PostSorting.Split_DataByReward
...
def plot_rewarded_spikes_on_track(recording_folder, spike_data, rewarded):
    ...
    for index, row in spike_data.iterrows():
        ...
        plt.xlim(0,200)
        Edmond.Ramp_analysis.Integrated_ramp_analysis.Python_PostSorting.plot_utility.style_track_plot(ax, 200)
        Edmond.Ramp_analysis.Integrated_ramp_analysis.Python_PostSorting.plot_utility.style_vr_plot(ax)
...
def plot_rewarded_spikes_on_track_with_tt(recording_folder,spike_data):
    ...
    for cluster in range(len(spike_data)):
        ...
        # split spikes by reward
        #spike_position_cm, spike_trial_numbers = split_spikes_by_reward(spike_data, cluster, rewarded_trials)
        rewarded_beaconed_position_cm, rewarded_nonbeaconed_position_cm, rewarded_probe_position_cm, rewarded_beaconed_trial_numbers, rewarded_nonbeaconed_trial_numbers, rewarded_probe_trial_numbers = Edmond.Ramp_analysis.Integrated_ramp_analysis.Python_PostSorting.Split_DataByReward.split_trials_by_reward(spike_data, cluster)
